...
シャード並列で更新するときは、**各シャードで `update --upload` を実行しない**でください。アーカイブ圧縮（zstd/gzip）では各シャードが自分のアーカイブを丸ごと上書きし、ダウンロード時に同名ファイルが後勝ちで競合して更新が失われます（ベースラインは痩せませんが更新が反映されません）。撮影だけをシャード並列にし、アップロードは集約します。

各シャードは `update --diff-only --shard N/T`（`--upload` なし）で撮影し、最後の集約ジョブで `download` → 全シャードの差分をマージ → 単一アーカイブで `upload` します。

```mermaid
flowchart TB
    C1["shard1: 差分A 撮影<br/>(upload しない)"] --> M["集約ジョブ"]
    C2["shard2: 差分B 撮影<br/>(upload しない)"] --> M
    M --> F["download 全件 → 差分マージ<br/>→ 単一アーカイブで upload"]
    F --> WIN["全件が最新で保たれる"]
```

> `compression: 'none'`（個別ファイル）なら、各シャードの `--upload` は PutObject の追記となり競合しないため、集約は不要です。
