...
export class LocalStorageAdapter implements StorageAdapter {
  ...
  async downloadFromGit(branch: string, destDir: string, cwd: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to list baselines from git branch '${branch}': ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    for (const file of files) {
      ...
      try {
        ...
        throw new Error(
          `Failed to extract '${file}' from git branch '${branch}': ${error instanceof Error ? error.message : String(error)}`,
        );
      }
